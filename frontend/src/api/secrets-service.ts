...
export class SecretsService {
  static async getSecrets() {
    const { data } = await openreplica.get<GetSecretsResponse>("/api/secrets");
  ...
  static async createSecret(name: string, value: string, description?: string) {
    ...
    const { status } = await openreplica.post("/api/secrets", secret);
    return status === 201;
  }
  ...
  static async updateSecret(id: string, name: string, description?: string) {
    ...
    const { status } = await openreplica.put(`/api/secrets/${id}`, secret);
    return status === 200;
  }

  static async deleteSecret(id: string) {
    const { status } = await openreplica.delete<boolean>(`/api/secrets/${id}`);
    return status === 200;
  }
  ...
  static async addGitProvider(providers: Record<Provider, ProviderToken>) {
    ...
    const { data } = await openreplica.post<boolean>(
