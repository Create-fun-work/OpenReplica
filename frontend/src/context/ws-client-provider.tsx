...
import { OpenReplicaParsedEvent } from "#/types/core";
import {
...
import { Conversation } from "#/api/openreplica.types";
...
const isOpenReplicaEvent = (event: unknown): event is OpenReplicaParsedEvent =>
  typeof event === "object" &&
  event !== null &&
  "id" in event &&
  "source" in event &&
...
const isCommandAction = (event: OpenReplicaParsedEvent): event is CommandAction =>
...
interface UseWsClient {
  ...
  parsedEvents: (OpenReplicaAction | OpenReplicaObservation)[];
...
export function WsClientProvider({
  ...
  const [parsedEvents, setParsedEvents] = React.useState<
    (OpenReplicaAction | OpenReplicaObservation)[]
  >([]);
  ...
  function handleMessage(event: Record<string, unknown>) {
    ...
    if (isOpenReplicaEvent(event)) {
      ...
      if (isOpenReplicaAction(event) || isOpenReplicaObservation(event)) {
